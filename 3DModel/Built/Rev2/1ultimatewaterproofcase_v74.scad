...
MetricScrewSize          = 5;     // [2:m2, 2.5: m2.5, 3: m3, 4: m4, 5: m5]
MetricScrewSizeTop       = 3;     // [2:m2, 2.5: m2.5, 3: m3, 4: m4, 5: m5]
...
DeviceHolder_X_Distance1  = 24.8;
// Distance between the holders in Y direction
DeviceHolder_y_Distance1  = 51;
...
/* [Wall CutOut settings side A ] */
// Activate customizable cut out
ShowSideWallCutOut_A        = true;
...
// Activate customizable holes for cable gland or similar
ShowSideWallHoles_D        = true;
// Count of holes, if there is an additional screw on X or Y side the hole in the middle is not showed
CountOfSideWallHoles_D     = 1;     //[1:1:3]
// Diameter of the holes
SideWallHoleDiameter_D     = 8;  //[1:0.1:80]
// Add or decrease height position (up and down, 0 = centered)
...
module BodyBottom () {
    if(ShowBottom)
    {
        difference(){
            ...
            if (ShowSideWallCutOut_A)
            {
                if (CountOfSideWallCutOut_A==1)
                {    translate([Caselength/2-SideWallThickness/2,SideWallCutOutPosition_A,SideWallCutOutOffset_Z_A+ BottomTopThickness+(CaseHeight-CutFromTop-BottomTopThickness)/2 ]) rotate([90,0,90]) cube([SideWallCutOutWidth_A,SideWallCutOutHeight_A, SideWallThickness+0.04],true);}
...
//                }
            ...
            if (ShowSideWallCutOut_B)
            {
                if (CountOfSideWallCutOut_B==1)
//                {    translate([Caselength/2-SideWallThickness/2,SideWallCutOutPosition_A,SideWallCutOutOffset_Z_A+ BottomTopThickness+(CaseHeight-CutFromTop-BottomTopThickness)/2 ]) rotate([90,0,90]) cube([SideWallCutOutWidth_A,SideWallCutOutHeight_A, SideWallThickness+0.04],false);}

                {    translate([SideWallCutOutPosition_B,-CaseWidth/2+SideWallThickness+.04,SideWallCutOutOffset_Z_B+ BottomTopThickness+(CaseHeight-CutFromTop-BottomTopThickness)/2 ]) rotate([90,0,0]) cube([SideWallCutOutWidth_B,SideWallCutOutHeight_B, SideWallThickness+0.08],false);}
...
module ShowSizes () {
    ...
    echo (str(" Screw counter sink  : ",ScrewCountersink,"mm "));
...
module BodyQuarterTop (Caselength,CaseWidth,CutFromTop,CaseRoundingRadius,SideWallThickness) {
    ...
    difference()
    {
        ...
        if (ScrewHeadType == 1)
        {
                echo (str("SrewHeadType 1"));
//            translate(ScrewCornerPos) ScrewCutTop(MetricScrewSizeTop,CutFromTop+0.01,0);
              translate(ScrewCornerPos) ScrewCutTop();
